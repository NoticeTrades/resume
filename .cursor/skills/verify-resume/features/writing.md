...
- **Path slug.** If a published article exists, run `control-resume browser goto --path '/writing/<slug>'`. It must not be a raw 404. The same title, body, and `← All musings` appear as `/writing/?article=<slug>`. The article body is visible (`.article-detail` is not stuck at `opacity: 0`).
